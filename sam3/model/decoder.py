...
class TransformerDecoder(nn.Module):
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        H, W = feat_size
        # 确保reference_boxes在正确的设备上
        reference_boxes = reference_boxes.to(reference_boxes.device)
        boxes_xyxy = box_cxcywh_to_xyxy(reference_boxes).transpose(0, 1)
        bs, num_queries, _ = boxes_xyxy.shape
        ref_device = reference_boxes.device  # 记录reference_boxes的设备
        
        if self.compilable_cord_cache is None:
            self.compilable_cord_cache = self._get_coords(H, W, ref_device)
        ...
        if torch.compiler.is_dynamo_compiling() or self.compilable_stored_size == (
            ...
            # 确保缓存的坐标在正确的设备上
            coords_h = coords_h.to(ref_device)
            coords_w = coords_w.to(ref_device)
        else:
