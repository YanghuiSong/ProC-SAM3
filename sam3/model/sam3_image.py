...
class Sam3Image(torch.nn.Module):
    ...
    def _get_img_feats(self, backbone_out, img_ids):
        ...
        if "backbone_fpn" in backbone_out:
            ...
            if (
                "_cached_img_feats" in backbone_out
                and "_cached_img_pos_embeds" in backbone_out
                and "_cached_vis_feat_sizes" in backbone_out
                and backbone_out["_cached_img_feats"]
                and img_ids.numel() > 0
            ):
                cached_batch = backbone_out["_cached_img_feats"][0].shape[1]
                min_img_id = int(img_ids.min().item())
                max_img_id = int(img_ids.max().item())
                if 0 <= min_img_id and max_img_id < cached_batch:
                    cached_img_feats = backbone_out["_cached_img_feats"]
                    cached_img_pos_embeds = backbone_out["_cached_img_pos_embeds"]
                    img_feats = [
                        feat.index_select(1, img_ids) for feat in cached_img_feats
                    ]
                    img_pos_embeds = [
                        pos.index_select(1, img_ids)
                        for pos in cached_img_pos_embeds
                    ]
                    return (
                        backbone_out,
                        img_feats,
                        img_pos_embeds,
                        backbone_out["_cached_vis_feat_sizes"],
                    )
    ...
    def forward_grounding(
        ...
        if self.training or self.num_interactive_steps_val > 0:
            if find_target is not None:
                ...
                # 如果find_target为None，我们跳过_matching步骤，但仍返回out
                # 这样可以让模型继续处理其他类别
                pass
        return out
    ...
    def back_convert(self, targets):
        if targets is None:
            ...
            return {
                ...
                "boxes_padded": torch.empty(0, 4, device=self.device),
                "positive_map": torch.empty(0, 1, device=self.device),
                "num_boxes": 0,
                "masks": torch.empty(0, 1, 1, device=self.device),  # 占位符
                "semantic_masks": torch.empty(0, 1, 1, device=self.device),  # 占位符
                "is_valid_mask": torch.empty(0, dtype=torch.bool, device=self.device)
            }
