...
class Attention(nn.Module):
    ...
    def _get_freqs_cis(self, rope_hw, device):
        if not self.use_rope:
            return None

        if self.rope_pt_size is None:
            self.rope_pt_size = rope_hw

        if self.input_size is not None and tuple(rope_hw) == tuple(self.input_size):
            assert self.freqs_cis is not None
            return self.freqs_cis.to(device)

        cache_key = tuple(rope_hw)
        if cache_key not in self._freqs_cis_cache:
            scale_pos = 1.0
            if self.rope_interp:
                scale_pos = self.rope_pt_size[0] / rope_hw[0]
            freqs_cis = self.compute_cis(
                end_x=rope_hw[0],
                end_y=rope_hw[1],
                scale_pos=scale_pos,
            ...
            if self.cls_token:
                ...
                cls_freqs_cis = torch.polar(torch.ones_like(t), t)[None, :]
                freqs_cis = torch.cat([cls_freqs_cis, freqs_cis], dim=0)
            self._freqs_cis_cache[cache_key] = freqs_cis.cpu()

        return self._freqs_cis_cache[cache_key].to(device)

    def _apply_rope(self, q, k, rope_hw) -> Tuple[Tensor, Tensor]:
        if not self.use_rope:
            return q, k

        freqs_cis = self._get_freqs_cis(rope_hw=rope_hw, device=q.device)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        q, k = self._apply_rope(q, k, rope_hw=(H, W))
        if self.use_rel_pos:
            q, k = concat_rel_pos(
                q.flatten(0, 1),
                k.flatten(0, 1),
