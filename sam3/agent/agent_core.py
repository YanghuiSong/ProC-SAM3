...
def agent_inference(
    ...
    debug: bool = False,
    model_path: str = "/data/public/Qwen3-VL-8B-Instruct",  # Added model path parameter
    device: str = "cuda" if torch.cuda.is_available() else "cpu",  # Added device parameter
    ...
    Args:
        ...
        model_path: Path to local Qwen3 model
    ...
    print("\n\n")
    generated_text = send_generate_request(messages, model_path=model_path, device=device)
    print(f"\n>>> MLLM Response [start]\n{generated_text}\n<<< MLLM Response [end]\n")
        ...
        if tool_call["name"] == "segment_phrase":
            ...
            for i in range(num_masks):
                ...
                checking_generated_text = send_generate_request(
                    iterative_checking_messages, model_path=model_path, device=device
                )
