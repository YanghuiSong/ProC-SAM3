...
def run_single_image_inference(
    ...
    text_prompt,
    llm_config,
    model_path="/data/public/Qwen3-VL-8B-Instruct",  # Added model path
    device="cuda" if torch.cuda.is_available() else "cpu",  # Added device
    ...
    agent_history, final_output_dict, rendered_final_output = agent_inference(
        image_path,
        text_prompt,
        model_path=model_path,  # Pass model path to agent_inference
        device=device,          # Pass device to agent_inference
